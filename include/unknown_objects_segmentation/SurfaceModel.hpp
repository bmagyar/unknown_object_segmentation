...
struct Edge {
  ...
  void Print() {
    std::cout << "  surfaces: " << surface[0] << "-" << surface[1] << std::endl;
    std::cout << "  corner: " << corner[0] << "-" << corner[1] << std::endl;
    std::cout << "  edge-size: " << edgels.size() << std::endl;
  }
};
...
class SurfaceModel
{
  public:
    ...
    void Print() {
      ...
      printf("  errors.size: %d\n  ", error.size());
      printf("  probs.size: %d\n  ", probs.size());
      printf("  coeffs.size %d: ", coeffs.size());
      ...
      printf("  neighbors2D [%d]: ", neighbors2D.size());
      for(size_t i=0; i<neighbors2D.size(); i++)
        printf("%d ", neighbors2D[i]);
      printf("\n  ");
      printf("  neighbors3D [%d]: ", neighbors3D.size());
      ...
      printf("  curves_image: %d\n", curves_image.size());
      printf("  curves_param: %d\n", curves_param.size());
#ifdef V4R_TOMGINE
